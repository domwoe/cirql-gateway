	...
	fbRef.child('externalTemperature').on('value', function(extTempObj) {
		...
		if (extTempObj.hasChild('timestamp') && extTempObj.hasChild('value') ) {
        	...
        	var upToDate = ((Date.now() - (timestamp - 5000)) < 20 * 60 * 1000) ? true : false;
  			...
  			console.log((Date.now() - timestamp)/1000/60);
			...
			self.hasExternalTempSensor(function(error, hasSensor) {
				...
				if (!error) {
					if (!hasSensor) {
						...
						log.info({host: hostname},'Defining virtual temperature sensor for '+that.id);
						...
						if (!error) {
							if (!isPeered) {
								if (upToDate) {
									...
									log.info({host: hostname},'Peering virtual temperature sensor with '+that.id);
							...
							// is peered
								...
								if (!upToDate) {
									...
									log.info({host: hostname},'Unpeering virtual temperature sensor from '+that.id);
...
Thermostat.prototype.peerVirtualSensor = function() {
	console.log('set extTemp_'+this.id+'_Btn1 peerChan 0 '+this.id+' single');
