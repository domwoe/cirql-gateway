...
function Thermostat(homeId, id) {
    ...
    this.fhemOffset = null;
    this.offsetIsFresh = false;
    ...
    this.fbRef.child('fhem_measured-temp').on('value', function(fbTrvTemp) {
        if (fbTrvTemp.val()) {
            ...
            if (this.usesInternalTemperature) {

                if (this.usesTemperatureOffset) {

                    this.useTemperatureOffset();
                }
            }
        }
    }, this);
    ...
    this.fbRef.child('usesInternalTemperature').on('value', function(fbUseIntTemp) {

        this.usesInternalTemperature = fbUseIntTemp.val();

        if (this.usesInternalTemperature === true) {
            if (this.isPeered === true) {


                var self = this;
                this.getPeeredSensor(function(error, sensorId) {

                    self.peeredSensor = sensorId;
                    if (sensorId + '' === self.virtualSensor + '' || sensorId + '' === 'extTemp_' + self.id + '_Btn1') {
                        self.isPeered = true;
        ...
        } else if (this.usesInternalTemperature === false) {

            if (this.virtualTempInterval) {
                clearInterval(this.virtualTempInterval);
            }

            if (this.extTempTimer) {
                clearTimeout(this.extTempTimer);
            }
    ...
    this.fbRef.child('usesTemperatureOffset').on('value', function(fbUseTempOffset) {

        this.usesTemperatureOffset = fbUseTempOffset.val();

        if (this.useTemperatureOffset === false) {
            this.setOffset(0);
        }
    ...
    this.fbRef.child('externalTemperature').on('value', function(extTempObj) {
        ...
        if (extTempObj.hasChild('timestamp') && extTempObj.hasChild('value')) {
            ...
            this.NetatmoUpToDate = ((Date.now() - this.extTempTimestamp) < 30 * 60 * 1000) ? true : false;
            ...
            } else { // use Netatmo peering

                this.useExternalTempSensor();
...
Thermostat.prototype.setOffset = function(offset) {

    if (this.offsetIsFresh !== true) {
        ...
        if (offset > 3.5) {
            ...
            offset = (Math.round(offset * 2) / 2).toFixed(1);
        }

        if (offset !== this.fhemOffset) {
            ...
            this.fbRef.child('offset').set(offset);



            fhem.write('set ' + this.id + '_Clima regSet tempOffset ' + offset + 'K\n');
            this.offsetIsFresh = true;
            var self = this;
            setTimeout(function() {
                self.offsetIsFresh = false;
            }, 15 * 60 * 1000);
        }
...
Thermostat.prototype.useTemperatureOffset = function() {
    ...
    if (this.fhemOffset !== null) {

        this.realOffset = Math.round((this.extTemp - (this.trvTemp - this.fhemOffset)) * 10) / 10;


        this.fbRef.child('realOffset').set(this.realOffset);

        this.setOffset(this.realOffset);

    }
};

Thermostat.prototype.useExternalTempSensor = function() {
    ...
    console.log('Thermmostat: ' + this.id);
    console.log('Temperature: ' + this.extTemp);
    console.log('Timestamp: ' + new Date(this.extTempTimestamp));
    console.log('Up to date: ' + this.NetatmoUpToDate);
                ...
                if (!error) {
                    ...
                    if (that.usesPrediction) {
                        ...
                        that.virtualTempInterval = setInterval(function() {
                            ...
                            extTempToSend = Math.round(extTempToSend * 10) / 10;
                    ...
                    if (that.NetatmoUpToDate && that.extTempTimer) {
                        // log.info({
                        //     host: hostname
        ...
        fhem[propertyFn](self.id, function(err, value) {
            if (!err) {
                if (typeof value != 'undefined') {
                    ...
                    } else if (property === 'tempOffset') {
                        console.log('FHEM TEMP OFFSET:' + value.Value);
                        if (value.Value.match('set') !== null ) {
                            self.fhemOffset = parseFloat((value.Value).split('K')[0]);
                            onsole.log('PARSED:' + self.fhemOffset);
                        }
                    }
