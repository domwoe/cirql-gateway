                        ...
                        if (!error) {
                            ...
                            // is peered
                                ...
                                if (!upToDate) {
                                    ...
                                    log.info({
                                        host: hostname
                                    }, 'Unpeering external temperature sensor from ' + that.id + ' because netatmo is not up to date. Last timestamp: ' + new Date(timestamp));
                            ...
                            var temperatureIncrement;
                            var extTempToSend;

                            if (newthis.lastExtTemp) {

                                extTempToSend = newthis.lastExtTemp;

                                temperatureIncrement = Math.round((newthis.extTemp - newthis.lastExtTemp) / 4 * 10) / 10;

                                extTempToSend = newthis.lastExtTemp + temperatureIncrement;

                            } else {

                                extTempToSend = newthis.extTemp;

                                temperatureIncrement = 0;
                            }

                            newthis.setExternalTemperature(extTempToSend);

                            newthis.virtualTempInterval = setInterval(function() {

                                if (temperatureIncrement >= 0) {

                                    if (extTempToSend + temperatureIncrement <= newthis.extTemp) {
                            ...
                            if (that.isPeered) {
                                ...
                                that.extTempTimer = setTimeout(function(self) {
                                    log.info({
                                        host: hostname
                                    }, 'Unpeering external temperature sensor from ' + self.id + ' because timer ran out. Last timestamp: ' + new Date(timestamp));
