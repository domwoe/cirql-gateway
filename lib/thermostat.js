...
function Thermostat(homeId, id) {
    ...
    this.fbRef('hasExternalTempSensor').set(false);
    this.fbRef('externalTempSensorIsPeered').set(false);
                ...
                this.setTargetInterval = setInterval(
                    function() {
                        if (self.retries <= 3) {
                            ...
                            this.status = 'retry ' + self.retries;
                            this.fbRef('status').set('retry ' + self.retries);
                            setTarget();
                            self.retries++;
                        } else {
                            log.warn({
                            ...
                            self.retries = 1;
                            this.status = 'max retries';
                            this.fbRef('status').set('max retries');
                        }
                    }, 5 * 60 * 1000);
            } else if (target === fhemTarget) {
                this.status = 'success';
                this.fbRef('status').set('success');
                log.info({
                        host: hostname,
                        thermostat: this.id
                    },
        ...
        // Manual change at Thermostat
            ...
            this.fbRef.child('target').set(fhemTarget, function(error) {
                if (error) {
                    ...
                    self.fbRef('status').set('manual change');
                }
            });
        } else {
            log.warn({
        ...
        if (extTempObj.hasChild('timestamp') && extTempObj.hasChild('value')) {
            ...
            self.hasExternalTempSensor(function(error, hasSensor) {
                ...
                if (!error) {
                    ...
                    self.fbRef('hasExternalTempSensor').set(true);
                    if (!hasSensor) {

                        self.fbRef('hasExternalTempSensor').set(false);
                    ...
                    that.isPeered(function(error, isPeered) {
                        ...
                        if (!error) {
                            if (!isPeered) {
                                that.fbRef('externalTempSensorIsPeered').set(false);
                            ...
                            // is peered
                                ...
                                that.fbRef('externalTempSensorIsPeered').set(true);
                                if (!upToDate) {
