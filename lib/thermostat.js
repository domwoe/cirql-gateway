...
function Thermostat(id,fbRef) {
	...
	this.firstAction1 = true;
	this.firstAction2 = true;
	...
	fbRef.child('target').on('value', function(target) {
		...
		fbRef.child('fhem_desired-temp').once('value', function(fhemTarget) {
				fhemTarget = null;
			if(!this.firstAction1) {
				this.firstAction1 = false;
	...
	fbRef.child('fhem_desired-temp').on('value', function(fhemTarget) {
		if(this.firstAction2) {
			this.firstAction2 = false;
			return;
		}
	...
	fbRef.child('externalTemperature').on('value', function(extTempObj) {
		...
		if (extTempObj.hasChild('timestamp') && extTempObj.hasChild('value') ) {
        	...
        	log.info({host: hostname},'Update from external temperature sensor for thermostat '+id+' Up to date: '+upToDate);
			...
			self.hasExternalTempSensor(function(error, hasSensor) {
				...
				if (!error) {
					if (!hasSensor) {
						...
						log.info({host: hostname},'Defining virtual temperature sensor for '+self.id);
					...
					that.isPeered(function(error, isPeered) {
						...
						log.info({host: hostname},'virtual sensor for '+that.id +' peered ? '+isPeered);
						...
						if (!error) {
							...
							if (upToDate && that.extTempTimer) {
								log.info({host: hostname},'Clearing unpeering timer for '+that.id);
								clearTimeout(that.extTempTimer);
							}

							if (isPeered) {
								log.info({host: hostname},'Setting unpeering timer for '+that.id);
...
Thermostat.prototype.activateBurst = function() {
	log.info({host: hostname},'Activate burst mode for '+this.id);
...
Thermostat.prototype.peerVirtualSensor = function() {
	log.info({host: hostname},'Peering virtual temperature sensor with '+this.id);
	fhem.write('set extTemp_'+this.id+'_Btn1 peerChan 0 '+this.id+' single\n');
};

Thermostat.prototype.unpeerVirtualSensor = function() {
	log.info({host: hostname},'Unpeering virtual temperature sensor from '+this.id);
...
Thermostat.prototype.setExternalTemperature = function(temperature) {
	log.info({host: hostname},'Set virtual temperature sensor for '+this.id+' with '+temperature);
	fhem.write('set extTemp_'+this.id+'_Btn1 virtTemp '+temperature+'\n');
};

Thermostat.prototype.deactivateWindowOpnMode = function() {
	log.info({host: hostname},'Deactivate window open mode for '+this.id);
	fhem.write('set '+this.id+'_Clima regSet winOpnMode off\n');
	fhem.write('set '+this.id+' getConfig\n');
};
		...
		fhem[propertyFn](self.id,function(err,value) {
			if (!err) {
				if (typeof value != 'undefined') {
					...
					else if (property === 'windowOpnMode' && (value.Value === 'on' || value.Value === 'on ') ) {
