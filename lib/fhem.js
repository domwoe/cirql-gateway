            ...
            if (line.length > 5) {
                ...
                if (fhemDeviceName.indexOf('HM_CC_RT_DN') > -1 && fhemDeviceName[0] !== 'e') {
                    if (re.test(fhemDeviceName)) {
                        ...
                        if ( fhemAttribute.toUpperCase().trim() === 'NACK') {
                            log.info({host: hostname, thermostat: thermostatId}, 'Got NACK from thermostat');
                        } 
                        else if ( fhemAttribute[fhemAttribute.length-1] === ':' && fhemAttribute !== 'T:') {
...
function getPeeredSensor(device,callback) {
    reqHttpApi(device,'Internals','peerList',function(err,value) {
        ...
        log.info({host: hostname}, 'getPeeredSensor:' +value + 'err: '+ err);
